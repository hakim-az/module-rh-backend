...
export class ContractGeneratorService {
  ...
  async generateMondialTVCDIContract(contart: any): Promise<Buffer> {
    ...
    // PAGE 1 (index 2)
    const engagamentFirst = wrapTextByLength(
      `Sous réserve des résultats de la visite médicale d'embauche, le salarié est engagé par la société MONDIAL TV, en qualité de ${contart.poste} dans les conditions prévues par les articles L. 7311-1 et suivants du code du travail.`,
      100
    );

    const engagamentSecond = wrapTextByLength(
      `Le présent contrat prendra effet le ${dateDebutFormatted}. Le salarié déclare expressément ce jour ne pas être débiteur d'une obligation de non-concurrence incompatible avec son engagement par la société MONDIAL TV en qualité de ${contart.poste}.`,
      100
    );
